...
# 打开cifar-10数据集文件目录
def unpickle(batch):
    with open("/tmp/data_batch_" + str(batch), 'rb') as fo:
...
def zip_file(postfix):
    ...
    zip_name = "/tmp/cifar-10-pictures-" + postfix + '.zip'
    src_dir = "/tmp/cifar-10-pictures-" + postfix
...
def trigger(num_workers, batch_size, epoches, l_r):
    ...
    #file_name = "data/cifar-10-batches-py.zip"
    #local_file = "/tmp/cifar-10-batches-py.zip"
    #s3_client.download_file("mybucket-zrj", file_name,
    #                        local_file)
    #unzip_file()
...
#    l_r = 0.05 #float(event['l_r'])
    ...
    for i in range(num_workers):
        if not os.path.exists('/tmp/cifar-10-pictures-' + str(i)):
            os.mkdir('/tmp/cifar-10-pictures-' + str(i))
            for j in range(10):
                os.mkdir('/tmp/cifar-10-pictures-' +
                        str(i) + '/' + str(label_name[j]))
    if not os.path.exists('/tmp/cifar-10-pictures-test'):
        os.mkdir('/tmp/cifar-10-pictures-test')
        for j in range(10):
            os.mkdir('/tmp/cifar-10-pictures-test/' + str(label_name[j]))
...
#            "cifar10-batches", "data_batch_"+str(batch), "/tmp/data_batch_"+str(batch))
        s3_client.download_file(
            "mybucket-zrj", "data/cifar-10-batches-py/data_batch_"+str(batch), "/tmp/data_batch_"+str(batch))
        ...
        for i in range(10000):
            ...
            cv2.imwrite("/tmp/cifar-10-pictures-" + str(j) + "/" + str(label_name[cifar_label[i]]) + "/" +
...
#        s3_client.upload_file(file_name, "traindata-bucket-chen", key)
    ...
    s3_client.download_file(
        "mybucket-zrj", "data/cifar-10-batches-py/test_batch", "/tmp/test_batch")
    with open("/tmp/test_batch", 'rb') as fo:
    ...
    for i in range(10000):
        ...
        img[:, :, 1] = g
        img[:, :, 2] = b
        cv2.imwrite("/tmp/cifar-10-pictures-test/" + str(label_name[cifar_label[i]]) + "/" +
...
def unzip_file():
    ...
    # dst_dir为生成的文件夹
    zip_name = "/tmp/cifar-10-batches-py.zip"
    dst_dir = "/tmp"
