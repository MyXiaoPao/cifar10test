...
#        output_file.write(f"Worker Index: {worker_index}\n")
        ...
        #unzip_file()
        train_name = "/tmp/cifar-10-pictures-" + str(worker_index)
        test_name = "/tmp/cifar-10-pictures-test"
        ...
        print("process_time", worker_index, end_time)
...
def scatter_reduce(weights, epoch, num_workers, worker_index, pattern_k, batch = 0):
    ...
    if worker_index < pattern_k:
        ...
        for index in range(num_workers):
            if index != worker_index:
                cur_file = '/tmp/tmp/' + tmp_prefix + str(index)
        ...
        merged_dir = '/tmp/merged/epoch' + str(epoch)
        try:
            os.mkdir(merged_dir)
    ...
    for index in range(pattern_k):
        if index != worker_index:
            cur_file = '/tmp/merged/epoch' + str(epoch) + '/' + merged_prefix + str(index)
            while not os.path.exists(cur_file):
                time.sleep(0.5)
            flag = True
            while flag:
